        ...
        findCssIncludes = function () {
            ...
            for (i = 0 ; i < document.styleSheets.length ; i += 1) {
                            ...
                            if (/^\.compilessinclude$/.test(cssRule.selectorText)) {
                                ...
                                if (matchBackgroundImage) {
                                    ...
                                    var backgroundImageUrl = URI(matchBackgroundImage[2]).absoluteTo(ownerNode.getAttribute('href')).absoluteTo(location.href).toString(),
                                        backgroundImageUrlWatchHref = cleanHref(backgroundImageUrl);
                                    if (href && backgroundImageUrlWatchHref) {
                                        cssIncludes.push({type: 'link', href: href, watchHref: backgroundImageUrlWatchHref, node: ownerNode});
                                    }
                                }
                            }
                        }
                ...
                if (liveStyleOptions.watchCssImages) {
                    ...
                    cssRules = styleSheet.rules || styleSheet.cssRules; // IE8 and below use .rules
                    if (cssRules) {
                        for (var k = 0 ; k < cssRules.length ; k += 1) {
                            cssRule = cssRules[k];
                            // Look for .compilessinclude {src: url(...);} in non-inline stylesheets:
                            style = cssRule.style;
                            for (var l = 0 ; l < style.length ; l += 1) {
                                ...
                                if (url && !(/^data:/.test(url))) {
                                    var cssImageUrl = URI(url).absoluteTo(baseUrl).absoluteTo(location.href).toString(),
                                        cssImageUrlWatchHref = cleanHref(cssImageUrl);
                                    if (href && cssImageUrlWatchHref) {
                                        cssIncludes.push({type: 'cssImage', href: href, watchHref: cssImageUrlWatchHref, cssRule: cssRule, propertyName: propertyName});
                                    }
                                }
                            }
                        }
